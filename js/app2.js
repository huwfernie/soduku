...
function setBoard(i){
  ...
  game.forEach((elem,index)=>{
    if(elem>0) {
      ...
      squares[index].classList.remove('fixedUser');
    }
  });
  return;
}
...
function setResetButton() {
  document.getElementById('resetGame').addEventListener('click', ()=>{
    const squares = document.getElementsByClassName('fixedUser');
    [].forEach.call(squares, (square)=>{
      square.innerHTML = '';
      square.classList.remove('fixedUser');
    });
  });
}
